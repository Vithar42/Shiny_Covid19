...
function(input, output, session) {
  ...
  # Message 1 StateBars tab  ----
  output$StateBarsmessage1 <- renderText({ 
    
    "Day 0 is the same as the cumulitive cases plot above, use the <Number of Infections for Day 0> slider to influacne this chart."
    
  })
  
  # Plot for StateBars tab ----
  output$StateBarsplot1 = renderImage({
    ...
    df <- statedata() %>%
      group_by(state) %>%
      select(state, value = deathpop, date) %>%
      arrange(desc(value)) %>%
      filter(value > 0)
    ...
    staticplot = ggplot(gdp, aes(rank, 
                                 group = state, 
    ...
    animate(anim, 100, fps = 3,  width = 1000, height = 1000, 
            renderer = gifski_renderer("outfile.gif"))
