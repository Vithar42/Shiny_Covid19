...
function(input, output, session) {
  ...
  output$StateTestingmessage <- renderText({ 
  ...
  # output$StateTestingplot1 <- renderPlotly({
  #   
  #   
  # })
  ...
  output$ui_StateTesting <- renderUI({
    ...
    do.call(tabsetPanel,
            lapply(state_choice,  function(state){
              ...
              tabPanel(title = state, state_ui_Testing_fun(state, input$logscaletoggle))
